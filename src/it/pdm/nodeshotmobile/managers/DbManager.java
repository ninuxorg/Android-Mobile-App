  ...
  //----------------GESTIONE DB ------------------------------------------//
    ...
    */
    ...
    public void truncateTable(String table) throws DBOpenException{
    	...
    	Log.v("PRIMA", "STO PER CHIUDERE DOPO AVER CANCELLATO");
        db.close();
        Log.v("DOPO", "HO CHIUSO");
    }
     ...
     */
    private void insertNodes(ArrayList<MapPoint> map) throws DBOpenException,SQLiteConstraintException{
    ...
    */
    ...
    private ArrayList<MapPoint> getNodeByType(String type) throws DBOpenException{
        ...
        if(type.equals("Hotspot")){
        	str_search="status = 'h' OR status = 'ah'";
        }
        ...
        //select * from Nodes where (lat between [min_lat] AND [max_lat]) and (lng between [min_lng] AND [max_lng]));
        String sql="SELECT * FROM "+DbHelper.TABLE_NAME1+" WHERE "+str_search;
        cursor= db.rawQuery(sql,null); //ottengo un cursore che punta alle entry ottenute dalla query
        cursor.moveToFirst();
        do{
        	if(cursor!=null && cursor.getCount()>0){
